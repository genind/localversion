...
<script>
	...
	function showCriticalError(pMessage, pTestName) {
	...
	function sendCheckRequest(pAction, pData, pSuccessCallback, pFailureCallback) {
		var action = pAction || "check";
		var data = pData || {};
		var successCallback = pSuccessCallback || processCheckingResults;
		var failureCallback = pFailureCallback || onRequestFailure;
		data["action"] = action;
	...
	function onRequestFailure(pReason)
	{
		var reason = pReason || "";
		showCriticalError("<?=GetMessageJS("SEC_SCANNER_TESTING_FAILURE")?>");
		completeTesting();
	}
	...
	function processCheckingResults(pResponce) {
		...
		if(!pResponce["status"]) {
			retrieveResults(pResponce);
		}

		if(pResponce["fatal_error_text"]) {
			showCriticalError(pResponce["fatal_error_text"], pResponce["name"]);
		}
		...
		if(pResponce["percent"]) {
			setProgress(pResponce["percent"]);
		}

		if(pResponce["name"]) {
			setCurrentTest(pResponce["name"]);
		}
	}
</script>
<?
